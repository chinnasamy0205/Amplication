...
describe("CategoryMgt2", () => {
  ...
  test("GET /categoryMgt2s/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/categoryMgt2s"}/${nonExistingId}`)
  ...
  test("GET /categoryMgt2s/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/categoryMgt2s"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /categoryMgt2s existing resource", async () => {
